...
/**
 * Generates an UpdateOne repository function for the given model name
 *
 ...
 */
function updateOneFactory(modelName) {
  ...
  return async function (_data) {
    try {
      ...
      const updateResult = await model.updateOne(data.query, data.updateValues);
...
module.exports = updateOneFactory;
